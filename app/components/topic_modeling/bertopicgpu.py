...
def bertopic_modeling():
    ...
    joblib.dump(embeddings, module_dir / "embeddings_fr.pkl")


    # Save everything
    topic_model.save(str(module_dir / "bertopic_model_fr"))
    joblib.dump(texts, module_dir / "texts_fr.pkl")
    joblib.dump(probs, module_dir / "probs_fr.pkl")
    ...
    bertopic_output_path = module_dir / "bertopic_output.csv"
    df.to_csv(bertopic_output_path, index=False)
    ...
    df = pd.read_csv(bertopic_output_path)
