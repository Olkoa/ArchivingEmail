...
cluster_levels_df = pd.read_csv("clusters_by_height_filtered.csv")
df4 = pd.read_pickle("df4_with_embeddings.pkl")
to_cluster_df=pd.read_csv("clustered_topics_tt.csv")
# Load later
linked = np.load("linked_matrix.npy", allow_pickle=True)
import os

output_dir = "outputs_by_height"
...
for col in cluster_levels_df.columns:
    if col.startswith("height_"):
        ...
        text_json_path = os.path.join(output_dir, f"clustered_texts_{height_str}.json")
        summary_json_path = os.path.join(output_dir, f"topic_summaries_{height_str}.json")
        tree_png_path = os.path.join(output_dir, f"cluster_tree_{height_str}")
...
def get_valid_heights(cluster_levels_df, summary_dir, all_heights):
    ...
    for h in all_heights:
        col = f"height_{h:.1f}"
        summary_file = os.path.join(summary_dir, f"topic_summaries_{h:.1f}.json")
        if col in cluster_levels_df.columns and os.path.exists(summary_file):
...
# Filter valid ones
heights = get_valid_heights(cluster_levels_df, "outputs_by_height", all_heights)
...
build_summary_tree_all_levels(
    ...
    summary_dir="outputs_by_height",
    heights=heights,
    output_path="outputs_by_height/full_tree_all_levels"
)
...
with open("grouped_topics_all_heights.json", "w") as f:
...
# Save result to JSON
with open("grouped_topics_filtered.json", "w") as f:
    json.dump(grouped_topics_by_height, f, indent=2)
import json
import os

# Load clusters_by_height from your JSON file
with open("grouped_topics_filtered.json", "r", encoding="utf-8") as f:
...
for h in sorted_heights:
    summary_file = os.path.join("outputs_by_height", f"topic_summaries_{h}.json")
...
with open("cluster_summary_tree.json", "w", encoding="utf-8") as f:
